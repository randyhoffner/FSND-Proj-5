...
APPLICATION_NAME = "Randy's Liquors App"
...
def categoriesJSON():
	...
	return jsonify (categories=[r.serialize for c in categories])
...
def deleteCategory(category_id):
	...
	if request.method == 'POST':
		session.delete(categoryToDelete.name)
		session.commit()
...
def newCategoryItem():
	...
	if login_session['user_id'] != category.user_id:
		...
		if request.method == 'POST':
			newItem = categoryItem(name=request.form['name'], description=request.form['description'], category_id=category_id, user_id=category.user_id)
...
def editCatgegoryItem(category_id, item_id):
	if 'username' not in login_session:
		return redirect('/login')
	...
	if request.method == 'POST':
		...
		return render_template('edititem.html', category_id=category_id, item=_id=item_id, item=editedItem)
